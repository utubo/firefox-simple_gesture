...
dlgs.gestureDlg = {
	FREE_FOR_EDIT: 999,
	onShow() {
		target = { name: dlgs.gestureDlg.targetId.replace(/_[^_]+$/, '') };
...
const setupGestureList = () => {
	...
	SimpleGesture.addTouchEventListener(byId('clearGesture'), { end: e => {
		updateGesture({ arrows: [CLEAR_GESTURE] });
		timeout.restore();
		history.back();
		safePreventDefault(e);
	}, start: safePreventDefault,  move: NOP, cancel: NOP, });
	SimpleGesture.addTouchEventListener($cancelInputGesture, { end: e => {
		timeout.restore();
		history.back();
		safePreventDefault(e);
	}, start: safePreventDefault, move: NOP, cancel: NOP, });
...
dlgs.editDlg = {
	targetId: null,
	async onShow() {
		const id = dlgs.editDlg.targetId;
...
const mySettings = {
	...
	onSavePre() {
		for (const key of Object.keys(SimpleGesture.ini)) {
			if (byId(key)?.classList?.contains('js-binding-exData')) {
				exData[key] = SimpleGesture.ini[key];
				delete SimpleGesture.ini[key];
			}
		}
		toggleExperimental();
		toggleDoubleTapNote();
